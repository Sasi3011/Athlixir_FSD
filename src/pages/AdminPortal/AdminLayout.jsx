...
import {
    LayoutDashboard, Users, UserCheck, Briefcase, GraduationCap, Calendar, FileText,
    BarChart2, MessageSquare, Settings, LogOut, Search, Bell, Menu, X, ShieldCheck, ChevronRight
} from "lucide-react";
...
const SIDEBAR_LINKS = [
    ...
    { name: "Verify Athletes", href: "/admin/verify", icon: UserCheck },
    { name: "Manage Opportunities", href: "/admin/opportunities", icon: Briefcase },
    { name: "Academy Approvals", href: "/admin/academies", icon: GraduationCap },
    { name: "Event Moderation", href: "/admin/events", icon: Calendar },
    { name: "System Logs", href: "/admin/logs", icon: FileText },
    { name: "Reports & Analytics", href: "/admin/reports", icon: BarChart2 },
...
const AdminLayout = () => {
    ...
    useEffect(() => {
        // Simple role check based on localStorage, adapt as necessary
        const role = localStorage.getItem('user_role');
        /* 
           // Commenting out strict check for dev purposes if needed, 
           // but normally this should be enabled.
           if (role !== 'admin') {
               navigate('/login'); 
           }
        */
        ...
        <div className="min-h-screen bg-[#050505] text-white flex overflow-hidden font-sans">
            ...
            <aside className={`hidden lg:flex flex-col bg-black/40 border-r border-white/5 transition-all duration-300 relative ${isSidebarOpen ? 'w-72' : 'w-20'}`}>
                ...
                <button
                    ...
                    className="absolute -right-3 top-8 z-50 w-6 h-6 bg-red-600 rounded-full flex items-center justify-center hover:scale-110 transition-transform shadow-lg shadow-red-600/30 border-2 border-[#050505]"
                    title={isSidebarOpen ? "Collapse sidebar" : "Expand sidebar"}
                >
                    <ChevronRight
                        size={14}
                        className={`text-white transition-transform duration-300 ${isSidebarOpen ? 'rotate-180' : ''}`}
                    />
                </button>
                ...
                <nav className="flex-1 px-4 py-4 space-y-1 overflow-y-auto scrollbar-hide">
                    {SIDEBAR_LINKS.map((link) => {
                        ...
                        return (
                            ...
                            >
                                <link.icon size={20} className={isActive ? "text-white" : "group-hover:text-red-500 transition-colors"} />
            ...
            <div className="flex-1 flex flex-col h-screen overflow-hidden">
                ...
                <header className="h-20 bg-black/20 border-b border-white/5 flex items-center justify-between px-6 lg:px-10 z-30">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="text-xl font-black uppercase tracking-widest text-white hidden md:block">
                    ...
                    <div className="flex items-center gap-4">
                        <button className="relative p-2.5 bg-white/5 rounded-xl border border-white/10 text-gray-400 hover:text-white transition-all">
                            <Bell size={18} />
                            <span className="absolute top-2 right-2 w-2 h-2 bg-red-600 rounded-full border-2 border-[#050505]"></span>
                        </button>
                        ...
                        <div className="flex items-center gap-3 pl-4 border-l border-white/10">
                            <div className="text-right hidden sm:block">
                                ...
                                <div className="flex items-center justify-end gap-1 mt-0.5">
                                    ...
                                    <span className="text-[9px] font-bold text-gray-500 uppercase tracking-widest leading-none">System Admin</span>
                                </div>
                            </div>
                            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-red-600 to-red-900 p-[1px]">
                                <div className="w-full h-full rounded-xl bg-black flex items-center justify-center text-red-600 font-black uppercase overflow-hidden border border-white/10 shadow-lg">
                                    A
                                </div>
                            </div>
                        </div>
